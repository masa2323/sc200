...
ネットワークには、Azure AD と同期するオンプレミスの Active Directory Domain Services (AD DS) ドメインが含まれています。AD DS ユーザーによる LDAP 要求を識別し、AD DS オブジェクトを列挙する必要があります。KQL  
...
この機能は、Kusto クエリ言語 (KQL) を使用して、ネットワーク内の未加工データ（テレメトリ）を最大 30 日分遡って探索できる強力なツールです。

- **証拠の収集:** 特定のアラートに関連するプロセス、ファイルの作成、ネットワーク接続などの詳細な挙動を、クエリによって自由自在に抽出・分析できます。
    
- **デバイスの特定:** 曖昧なインジケーターからでも、条件に合致するデバイスを迅速に洗い出すことが可能です。
    
- **アクションの実行:** 最も重要な点として、**クエリの結果画面から直接「デバイスの分離 (Isolate Device)」などの応答アクションを実行できる**仕組みが備わっています。