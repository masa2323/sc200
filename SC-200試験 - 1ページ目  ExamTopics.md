...
**正解:** 
DeviceLogonEvents | where DeviceName in ("CFOLaptop", "CEOLaptop", "COOLaptop") | where ActionType == "LogonFailed" | summarize LogonFailures=count() by DeviceName, LogonType

**解説:**
- **テーブルの選択**: サインイン（ログオン）活動を調査するため、専用の `DeviceLogonEvents` テーブルをソースとして指定します。
    
- **デバイスの絞り込み**: `| where` 句と `in` 演算子を使用して、対象の 3 デバイスにフィルタリングします。
    
- **失敗イベントの抽出**: 回答エリアにある `and` に続けて `ActionType == "LogonFailed"` を指定することで、「指定したデバイス」かつ「サインイン失敗」という条件を完成させます。
    
- **集計処理**: 「カウントする」という目的に対し、`summarize` 句と `count()` 関数を用いて、デバイスおよびログオン種別ごとの合計を算出します。
...
Microsoft 365 サブスクリプションをご利用で、Microsoft Defender for Office 365 を使用しています。Microsoft SharePoint Online サイトには機密文書が含まれています。これらの文書には、32 文字の英数字からなる顧客アカウント番号が含まれています。  
...
**A. アラートを自動的に解決します** **B. アラートを非表示にします** **D. デバイス グループを対象とする抑制ルールを作成します**

## 解説

### なぜこれらが正解か：

1. **アラートを非表示にする（B）** - Microsoftのドキュメントによると、アラートチューニング（旧：アラート抑制）の**Action**で「**Hide alert**」（アラートを非表示にする）または「**Resolve alert**」（アラートを自動的に解決する）を選択できます。誤検知の場合、アラートを非表示にすることで、アラートキューのノイズを減らすことができます。
2. **アラートを自動的に解決する（A）** - アラートチューニングルールでは「**Resolve alert**」アクションを選択でき、条件に一致するアラートが自動的に解決されます。これにより、既知の良性活動によるアラートが自動的に処理されます。
3. **デバイス グループを対象とする抑制ルールを作成する（D）** - 経理チームのデバイスという特定のグループに限定して抑制ルールを作成することで、他のデバイスのセキュリティ体制に影響を与えずに、対象デバイスのみで誤検知を抑制できます。抑制ルールの「**Select scope to apply to**」（適用するスコープの選択）で特定のデバイスグループを指定できま
...
新たなランサムウェアの亜種を展開する潜在的な攻撃を調査しています。
3 つのカスタムデバイスグループがあり、これらのグループには機密性の高い情報が保存されているデバイスが含まれています。  
...
Microsoft Defender for Identity と Active Directory の統合を構成しています。Microsoft Defender for Identity ポータルから、攻撃者が悪用できる複数のアカウントを構成する必要があります。  
...
Microsoft Defender for Identityのドキュメントによると、**ハニートークン（Honeytoken）アカウント**は以下の目的で使用されます：

- **攻撃者の罠として機能** - ハニートークンアカウントは通常休眠状態であり、正常な業務では使用されません
- **攻撃検知のトリガー** - ハニートークンアカウントに関連する認証活動があると、自動的にアラートがトリガーされます
- **攻撃者が悪用できるアカウント** - 問題文の「攻撃者が悪用できる複数のアカウントを構成する必要があります」という要件に正確に合致します

### エンティティタグからの設定方法

Microsoftのドキュメントには以下の手順が記載されています：

1. **Microsoft Defender XDR**ポータルにサインイン
2. **設定 > ID（Identities）**を選択
3. **Honeytoken**タグタイプを選択
4. **Tag users**（ユーザーにタグ付け）ボタンをクリック
5. タグ付けしたいエンティティを選択して追加

### サポート対象

- **Honeytoken**タグは、**ユーザーとデバイス**の両方をサポートします
- 複数のアカウントを一度に選択してハニートークンとしてタグ付けできます

### アラート機能

ハニートークンアカウントとしてタグ付けされたアカウントで認証活動が発生すると、以下のようなアラートが生成されます：

- **Honeytoken Activity**（重大度: High）
- **Honeytoken user attributes modified**（外部ID: 2427）
- **Honeytoken group membership changed**（外部ID: 2428）
- **Honeytoken was queried via LDAP**（外部ID: 2429）