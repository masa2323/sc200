...
ユーザーから、添付ファイルを含むメールメッセージの受信に予想よりも時間がかかるという報告を受けています。セキュリティを損なうことなく、添付ファイルを含むメッセージの配信時間を短縮する必要があります。添付ファイルはマルウェアスキャンの対象となり、マルウェアを含むメッセージはブロックする必要があります。  
...
機密ファイルが外部で共有された場合にアラートを生成し、修復アクションをトリガーするようにMicrosoft Cloud App Securityを構成する必要があります。Cloud App Securityポータルで実行する必要がある2つのアクションはどれですか？ 正解はそれぞれ解答の一部を示しています。  
注：正解は1つにつき1ポイントです。  
...
1. ✅ **ファイル監視を有効にする**（選択肢E）
2. ✅ **Microsoft Information Protection統合を有効にする**（選択肢D）
3. **ファイルポリシーを作成する**：
    - **ポリシー > ポリシー管理 > ファイルポリシーを作成**
    - **アクセスレベル = 外部/パブリック**でフィルター
    - **感度ラベル = 機密**でフィルター
    - **ガバナンスアクション**を設定（例：外部ユーザーの削除、アラートの生成）
