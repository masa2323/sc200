...
ご提示いただいた問題の正解は **B. registry** です。

Microsoft Defender for Endpoint の**ライブ応答 (Live Response)** セッションにおいて、レジストリ操作を直接行うための専用コマンドを選択する問題です。
---
### 正解の理由：B. registry

ライブ応答セッションでは、リモートのデバイスに対してリアルタイムでコマンドを実行できます。レジストリ値を読み取ったり、今回のように**変更（元に戻す）**したりするには、`registry` コマンドを使用します。

- **用途:** レジストリキーの表示、作成、削除、および値の修正が可能です。
    
- **具体例:** 攻撃者がウイルス対策の除外設定を悪用してレジストリを書き換えた場合、`registry set` などのサブコマンドを用いて、正しい値へ修正（ロールバック）することができます。
    
---
### 他の選択肢が適さない理由

- **A. 修復する (remediate):** ライブ応答には `remediate` という名前のコマンドは存在しません。エンティティの「修復」は、自動調査 (AIR) やポータル上のアクションメニューから行われる一連のワークフローを指します。
    
- **B. スキャン (scan):** デバイス上で Microsoft Defender ウイルス対策のスキャンを開始するためのコマンドです。レジストリの値を直接書き換える機能はありません。
    
- **D. 分析する (analyze):** ライブ応答には `analyze` という直接的なコマンドはなく、特定のエンティティ（ファイルなど）の調査は他の手法で行います。
    
---
### 💡 試験・実務でのポイント

ライブ応答で使用頻度の高いコマンドは、試験対策としても重要です。

| **コマンド**       | **役割**                    |
| -------------- | ------------------------- |
| `getfile`      | 調査のためにデバイスからファイルをダウンロードする |
| `putfile`      | デバイスにスクリプトやツールをアップロードする   |
| `run`          | アップロードしたスクリプトを実行する        |
| `processes`    | 実行中のプロセス一覧を表示する           |
| **`registry`** | **レジストリの参照・編集を行う**        |

質問#24 トピック7

オンプレミス ネットワークには、contoso.com と fabrikam.com という 2 つの Active Directory ドメイン サービス (AD DS) ドメインが含まれています。Contoso.com には Group1 というグループが含まれています。Fabrikam.com には Group2 というグループが含まれています。WS1 という Microsoft Sentinel ワークスペースがあり、そこには Rule1 というスケジュールされたクエリ ルールが含まれています。Rule1 は、異常な AD DS セキュリティ イベントに応答してアラートを生成します。各アラートはインシデントを作成します。  
