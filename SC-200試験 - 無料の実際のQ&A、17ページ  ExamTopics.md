...
**解説:**
ワークブックでインシデント一覧を表示し、各インシデントの最新ログのみを含めるクエリです。

1. `Summary = arg_max(TimeGenerated, *) by IncidentNumber`: **arg_max** 関数は、指定されたカラム（TimeGenerated）が最大である行（最新の行）を、グループ（IncidentNumber）ごとに返します。これにより、インシデントごとの最新の状態を取得できます。
2. `sort by IncidentNumber desc`: インシデント番号順（降順または昇順）に並べ替えます。
...
**解説:**
重複イベントを避けつつ、各リソースからイベントを収集するための最適な構成です（Azure Monitor Agentへの移行を前提とした回答と考えられます）。

1. **Azure virtual machines (SW1)**: **「Azure Monitor Agent (AMA)」**。Azure VMにはAMA拡張機能をインストールし、データ収集ルール(DCR)で収集対象を定義します。
2. **Non-Azure virtual machines (SW2)**: **「Azure Arc」**。オンプレミスや他クラウドのVMは、Azure Arc対応サーバーとしてオンボードすることで、Azure VMと同様にAMAをデプロイ・管理できます。
3. **Network devices (SW3)**: **「Azure Monitor Agent」** (LinuxベースのSyslogフォワーダー)。ネットワーク機器からのSyslog/CEFは、AMAをインストールしたLinuxフォワーダー経由でSentinelに送信します。
※MMA (Microsoft Monitoring Agent) とAMAを併用するとログが重複する可能性があるため、移行期間以外はどちらかに統一（現在はAMA推奨）することが重要です。
